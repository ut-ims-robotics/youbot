...
public:
    ...
    float duration = 3;                                       // Duration of recording in second
    ...
    void refineTrajectory()
    {
        ...
        for (int i=0; i<goals[activeGoal].trajectory.points.size()-1; i++)
        {
            float timeDifference = goals[activeGoal].trajectory.points[i+1].time_from_start.toSec() - goals[activeGoal].trajectory.points[i].time_from_start.toSec();
                ...
                if (timeDifference > k*gapMultiplier*shortestPeriod)
                {
                    ...
                    else
                    {
                        ...
                        for(int joint=0; joint<numberOfJoints; joint++)
                        {
                            ...
                            vel1[joint] = (points[i+2].positions[joint] - points[i+1].positions[joint]) /
    ...
    {
        ROS_INFO("Displaying the trajectory of the active goal.");
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub_traj_goal = n.subscribe("/arm_1/arm_controller/follow_joint_trajectory/goal", 1, &TrajectoryRecorder::trajectoryCallback, &recorderT);
