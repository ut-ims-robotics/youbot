...
public:
    ...
    float duration = 10;                                       // Duration of recording in second
    ...
    trajectory_msgs::JointTrajectory handoverTrajectory;
    trajectory_msgs::JointTrajectory closerTrajectory;
    trajectory_msgs::JointTrajectory boxDemoTrajectory;
    ...
    {
        // Move the arm into starting state
        ...
        {
            armJointPositions[i].joint_uri = goals[activeGoal].trajectory.joint_names[i];
            armJointPositions[i].value = goals[activeGoal].trajectory.points[0].positions[i];
            armJointPositions[i].unit = boost::units::to_string(boost::units::si::radians);
        }

        ROS_INFO("startTrajectory: Moving to the starting point of the trajectory");
...
*/
    ...
    bool executeTrajectoryService(youbot_manual_operation::executeTrajectory::Request &req,
                                  youbot_manual_operation::executeTrajectory::Response &res)
    {
        if (req.executionType == 1)
        {
            res.executionTypeResp = req.executionType;
            sendHackedTrajectory( boxDemoTrajectory, req.joint1Changed, req.joint1Value);
        }
        else if (req.executionType == 2)
        {
            res.executionTypeResp = req.executionType;
            sendHackedTrajectory( handoverTrajectory, req.joint1Changed, req.joint1Value);
        }
        else if (req.executionType == 3)
        {
            res.executionTypeResp = req.executionType;
            sendHackedTrajectory( closerTrajectory, req.joint1Changed, req.joint1Value);
        }
        else
            res.executionTypeResp = 999;

        return true;
    }

    void getDisplayTrajectoryCallback(const moveit_msgs::DisplayTrajectory & msg)
    {
        boxDemoTrajectory = msg.trajectory[0].joint_trajectory;
        ROS_INFO("getDisplayTrajectoryCallback: Got a new trajectory");
    }

    // JUST A HACK FOR SENDING TRAJECTORIES
    ...
    void trajectoryCallbackHandover(control_msgs::FollowJointTrajectoryActionGoal msg)
    {
        ROS_INFO("Recieved a handover goal! Adding it to goals list.");
        handoverTrajectory = msg.goal.trajectory;
        control_msgs::FollowJointTrajectoryGoal goal;
        goal = msg.goal;
        activeGoal = goals.size();
        goals.push_back(goal);
    ...
    void trajectoryCallbackCloser(control_msgs::FollowJointTrajectoryActionGoal msg)
    {
        ROS_INFO("Recieved a closer goal! Adding it to goals list.");
        closerTrajectory = msg.goal.trajectory;
        control_msgs::FollowJointTrajectoryGoal goal;
        goal = msg.goal;
        activeGoal = goals.size();
        goals.push_back(goal);
    ...
    void keyboardCallback(keyboard_reader::Key kbCommand)
    {
        if (kbCommand.key_pressed == true)
        {
            ...
            {
                trajectoryCheck(goals[activeGoal]);
            }

            // Refine the trajectory: "f" key
            else if (kbCommand.key_code == 0x0021)
            {
                refineTrajectory();
            }

            // send hacked trajectory: "8" key
            else if (kbCommand.key_code == 0x0009)
            {
                sendHackedTrajectory( goals[activeGoal].trajectory , false, 0);
            }
        }
    }
};
...
int main(int argc, char** argv)
{
    ...
    armPositionsPublisher = n.advertise<brics_actuator::JointPositions > ("arm_1/arm_controller/position_command", 100);
    ...
    ros::Subscriber sub_traj_goal_handover = n.subscribe("/arm_1/arm_controller/follow_joint_trajectory/goalBagHandover", 1, &TrajectoryRecorder::trajectoryCallbackHandover, &recorderT);
    ros::Subscriber sub_traj_goal_closer = n.subscribe("/arm_1/arm_controller/follow_joint_trajectory/goalBagCloser", 1, &TrajectoryRecorder::trajectoryCallbackCloser, &recorderT);
    ros::Subscriber sub_disp_traj = n.subscribe("move_group/display_planned_path", 10, &TrajectoryRecorder::getDisplayTrajectoryCallback, &recorderT);
    ros::ServiceServer service = n.advertiseService("execute_trajectory", &TrajectoryRecorder::executeTrajectoryService, &recorderT);
